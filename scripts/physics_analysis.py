import sysconfig
import os
from utils import make_sure_path_exists, printer, flatten_dict, backup_and_save
import vcode
...
def read_vcode_coding(Exp, filter={}):
	'''Check all sessions for this study for expected VCode files. Add to coding data under 'vcode' field and save.
	TODO: full doc'''

	sessionKeys = Exp.filter_keys(Exp.coding, filter)
	for sessKey in sessionKeys:
		codeRec = Exp.coding[sessKey]
		...
		for coderName in theseCoders:
			...
			if 'vcode' in codeRec.keys():
				...
				codeRec['vcode'] = {coderName: vcodeData}
			Exp.coding[sessKey] = codeRec

	# Save coding data
	backup_and_save(paths.coding_filename(Exp.expId), Exp.coding)

def summarize_results(Exp):
	'''Summarize some coding results from an Experiment object - for physics studies'''
